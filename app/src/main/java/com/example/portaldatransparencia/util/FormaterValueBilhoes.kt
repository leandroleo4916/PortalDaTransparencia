...
class FormaterValueBilhoes {
    ...
    fun formatValor(valor: Double): String {
        ...
        return when (s.length){
            0 -> {
                val value = ss.split(".")
                if (value[1].length != 1) "$ ${value[0]},${value[1]}"
            ...
            3 -> {
                val value = ss.split(".")
                if (value[1].length != 1) "R$ ${value[0]},${value[1]}"
                else "R$ ${value[0]},${value[1]}0"
            }
            4 -> {
            ...
            8 -> {
                ...
                val sub2 = s.substring(2, 3)
                "R$ $sub1,$sub2 mi"
            }
            9 -> {
                val sub1 = s.substring(0, 3)
                val sub2 = s.substring(3, 4)
                "R$ $sub1,$sub2 mi"
            }
            10 -> {
                val sub1 = valor.toString().substring(0, 1)
                val sub2 = valor.toString().substring(2, 3)
                "R$ $sub1.$sub2 bi"
            }
            else -> {
                val sub1 = s.substring(0, 3)
